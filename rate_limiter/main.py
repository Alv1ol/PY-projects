...
from fastapi import FastAPI, Request, HTTPException, status, Request, Body, Depends
...
class RateLimiter:
    ...
    async def is_limited(
        ...
        current_request = f"{current_ms}:{random.randint(0, 1000000)}"
...
rate_limit_sql = rate_limiter_factory(
    endpoint="sql_code",
    max_requests=5,
    window_seconds=5,
)
rate_limit_python = rate_limiter_factory(
